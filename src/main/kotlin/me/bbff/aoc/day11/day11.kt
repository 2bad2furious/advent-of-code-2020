...
data class Seats(
    private val seats: List<List<Seat>>
) {
    private val rowMaxIndex = seats[0].lastIndex

    fun seatsInAllDirections(x: Int, y: Int): Sequence<Sequence<Seat>> = sequence {
        yieldAllInDirection(x, y, -1, -1)
        yieldAllInDirection(x, y, 0, -1)
        yieldAllInDirection(x, y, 1, -1)

        yieldAllInDirection(x, y, -1, 0)
        yieldAllInDirection(x, y, 1, 0)
    ...
    suspend fun SequenceScope<Sequence<Seat>>.yieldAllInDirection(x: Int, y: Int, stepX: Int, stepY: Int) {
        yield(
            sequence {
                var curX: Int = x
                var curY: Int = y
                do {
                    curX += stepX
                    curY += stepY
                } while (yieldIfPresent(curX, curY))
            }
        )
    }

    suspend fun SequenceScope<Seat>.yieldIfPresent(x: Int, y: Int): Boolean {
        val isValid = x >= 0 && y >= 0 && y <= seats.lastIndex && x <= rowMaxIndex
        if (isValid) {
            yield(seats[x, y])
        }
...
fun part1(input: List<List<Seat>> = realInput.parsed): UInt {
    var oldState: List<List<Seat>>
    val newState: List<MutableList<Seat>> = input.map { it.toMutableList() }

    do {
        oldState = newState.map { it.toList() }
        val oldSeats = Seats(oldState)
        newState.forEach { x, y ->
            val seat = oldState[x, y]
            val count = oldSeats.seatsInAllDirections(x, y)
                .mapNotNull { it.firstOrNull() }
                .count { it == Seat.OCCUPIED }
            when {
                seat == Seat.EMPTY && count == 0 -> newState[x, y] = Seat.OCCUPIED
                seat == Seat.OCCUPIED && count >= 4 -> newState[x, y] = Seat.EMPTY
            }
        }
    } while (oldState != newState)


    return newState.sumOf { it.count { s -> s == Seat.OCCUPIED } }.toUInt()
}

fun part2(input: List<List<Seat>> = realInput.parsed): UInt {
    var oldState: List<List<Seat>>
    val newState: List<MutableList<Seat>> = input.map { it.toMutableList() }
    ...
    do {
        ...
        newState.forEach { x, y ->
            ...
            val count = oldSeats.seatsInAllDirections(x, y)
                .countUnsigned {
                    it.firstOrNull { s -> s != Seat.FLOOR } == Seat.OCCUPIED
                }
            when {
                seat == Seat.EMPTY && count == 0u -> newState[x, y] = Seat.OCCUPIED
                seat == Seat.OCCUPIED && count >= 5u -> newState[x, y] = Seat.EMPTY
            }
        }
    } while (oldState != newState)
...
fun List<List<Seat>>.asString() = buildString {
    for (row in this@asString) {
        appendLine(row.joinToString(""))
    }
}

operator fun List<List<Seat>>.get(x: Int, y: Int): Seat = get(y)[x]
operator fun List<MutableList<Seat>>.set(x: Int, y: Int, seat: Seat) {
    get(y)[x] = seat
}

inline fun List<List<Seat>>.forEach(block: (x: Int, y: Int) -> Unit) {
    for (y in indices)
...
val String.parsed
    get() = splitToSequence('\n')
        .map { it.map { seat -> Seat.fromChar(seat) } }
        .toList()

val simpleInput = """L.LL.LL.LL
LLLLLLL.LL
L.L.L..L..
LLLL.LL.LL
L.LL.LL.LL
...
val simpleStagesPart2 = """L.LL.LL.LL
LLLLLLL.LL
L.L.L..L..
LLLL.LL.LL
L.LL.LL.LL
...
#.L#LL#.L#"""
    .splitToSequence("\n\n")
    .map { stage ->
        stage.splitToSequence('\n')
            .map { row ->
                row.map { Seat.fromChar(it) }
            }.toList().asString()
    }.toList()

val realInput = """LLLLL.LLL.LLLLLLLLLLLLLLL.LLLLLLLLL.LLLLLLLLLLLLL.LLLLLLL.LLLLLLL.LLLLL.LLLLLLLLLLLLLLLLLLL.LLLLL
