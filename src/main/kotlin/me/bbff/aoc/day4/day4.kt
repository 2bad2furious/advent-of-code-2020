package me.bbff.aoc.day4

import me.bbff.utils.countUnsigned

val rawInput = """iyr:2015
...
val people: Sequence<Sequence<Pair<String, String>>> = rawInput.splitToSequence("\n\n").map { personData ->
    personData.split(whitespace).asSequence().map { field ->
...
val hairColorRegex = """^#[\da-f]{6}$""".toRegex()
val eyeColorOptions = arrayOf("amb", "blu", "brn", "gry", "grn", "hzl", "oth")
val passportIdRegex = """^\d{9}$""".toRegex()

val requiredFieldsWithValidation = mapOf<String, (String) -> Boolean>(
    "byr" to {
        val year = it.toIntOrNull()
        year != null && year in 1920..2002
    },
    "iyr" to {
        val year = it.toIntOrNull()
        year != null && year in 2010..2020
    }, "eyr" to {
        val year = it.toIntOrNull()
        year != null && year in 2020..2030
    }, "hgt" to {
        if (it.length >= 4) {
            val unitIndex = it.length - 2
            val amount = it.substring(0, unitIndex).toIntOrNull()
            when (it.substring(unitIndex)) { // unit
                "in" -> amount in 59..76
                "cm" -> amount in 150..193
                else -> false
            }
        } else false
    }, "hcl" to {
        it matches hairColorRegex
    }, "ecl" to {
        eyeColorOptions.contains(it)
    }, "pid" to {
        it matches passportIdRegex
    }
)
...
fun part1(data: Sequence<Sequence<Pair<String, String>>> = people): UInt {
    val requiredKeys = requiredFieldsWithValidation.keys
    return data.countUnsigned {
        it.map(Pair<String, String>::first).toList().containsAll(requiredKeys)
    }
}

fun part2(data: Sequence<Sequence<Pair<String, String>>> = people): UInt {
    return data.countUnsigned {
        val personData = it.toMap()
        requiredFieldsWithValidation.all { (key, validation) ->
            when (val value = personData[key]) {
                null -> false
